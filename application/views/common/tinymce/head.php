...
<html lang="en">
    <head>
        ...
        <meta name="author" content="Samuel Stradling" />
        <meta name="author" content="Rupert Maspero" />
        <meta name="author" content="Courney Edgar" />
                ...
                <div id="header-width">
                    <div id="header-top">
                        ...
                        <div id="login-details">
                            <?php if(logged_in()) {
                                echo anchor('details/profile', '<div id="header-profile" style="background-image: url('.get_usr_img_src($this->session->userdata('uid'), 'tiny').')" ></div>'); ?>
                                <div id="header-list">
                                    <p><?php echo anchor('details/profile', user_pref_name($this->session->userdata('firstname'),$this->session->userdata('prefname'))).'<br />'.(is_admin() ? anchor('admin', 'Website Admin').'<br />' : '').($this->session->userdata('current') ? '' : anchor('alumni', 'Alumni').'<br />').anchor('home/logout', 'Logout'); ?></p>
                                </div>
                            <?php } else { ?>
                    ...
                    <div id="nav" role="navigation" class="<?php echo $class;?>">
                        ...
                        ?>
                                    <div class="heading">
                                        <h2 <?php echo is_null($d['link'])?'':' class="link-head"'; ?>><?php echo link_and_name($m['title']); ?></h2>
                                        <table>
                                            <?php
                                                foreach($m as $k => $e){
                                                    if(is_numeric($k)){
